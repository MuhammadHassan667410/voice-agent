...
def _to_product_item(row: dict[str, Any], include_score: bool = False) -> ProductItem:
...
def search_products(
    ...
    items = [_to_product_item(row, include_score=True) for row in rows]
...
def get_product(
    ...
    return ProductResponse(trace_id=trace_id, item=_to_product_item(row))
...
def filter_products(
    ...
    items = [_to_product_item(row, include_score=False) for row in rows]
    return SearchResponse(
        trace_id=trace_id,
        items=items,
        page=PageMeta(limit=body.pagination.limit, offset=body.pagination.offset, total_estimate=len(items)),
